...
import { editorStore, updateDraftContent, saveFileContent, IEditorContent } from '@/components/editor/stores/editorStore'; 
...
const FileEditorViewer: React.FC<FileEditorViewerProps> = ({
  ...
  const isLoading = isContextualMode ? contextIsLoading : globalState.isLoading;
  ...
  if (isLoading) {
    return (
        <Box className="flex flex-col items-center justify-center h-full">
            <CircularProgress />
            <Typography variant="h6" sx={{ mt: 2 }}>Loading {fileEntry?.name}...</Typography>
        </Box>
    );
  }
