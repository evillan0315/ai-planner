...
export const updateCurrentPlanMetadata = (updatedMetadata: {
  ...
  if (current.plan && current.currentPlanId === current.plan.id) {
    // We must ensure that properties like tests, metadata, and error are preserved
    // if not explicitly updated.
    plannerStore.set({
      ...current,
      plan: {
        ...current.plan,
        // Ensure changes array and required ID/Title are preserved
        id: current.plan.id,
        title: updatedMetadata.title ?? current.plan.title,
        changes: current.plan.changes,
