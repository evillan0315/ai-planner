...
export interface IFileChange {
  ...
  // Optional fields, may be null from DB or omitted by LLM
  ...
  estimatedMinutes?: number; // Added from backend DTO
}

export interface IGitInstructions {
  branchName: string;
 ...
 * Aligns with the backend's GeneratedPlanDto (persisted entity).
 */
export interface IPlan {
  ...
  // LLM Output fields
  summary?: string;
  thoughtProcess?: string[] | string; // Can be string[] (LLM) or string (DB fetch)
  assumptions?: string[]; // Added
  confidence?: number; // Added
  estimatedEffortMinutes?: number; // Added
  documentation?: string; // Content of the linked Documentation entity
  buildScripts?: Record<string, string>; // Added based on schema/usage context
  gitInstructions?: IGitInstructions; // Structured Git instructions
  changes: IFileChange[];
  tests?: ITests; // Structured tests
  metadata?: IMetadata; // Structured metadata
  error?: string | null; // Error message if LLM failed to produce a valid plan

  // Persisted/Contextual fields
  documentationId?: string; // ID of the linked Documentation entity (optional, internal)
  ...
  projectRoot?: string; // Extracted project root from llmInput
  planId?: string; // Redundant plan identifier, sometimes present
}
